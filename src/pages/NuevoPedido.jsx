    ...
    for (var i = 0; i < archivos.length; i++) {
      ...
      try {
        ...
        if (archivo.name.toLowerCase().endsWith('.xls') && !archivo.name.toLowerCase().endsWith('.xlsx')) {
          setProgresoParseo('Convirtiendo ' + archivo.name + '...')
          if (!window.XLSX) {
    ...
    for (var i = 0; i < parseado.articulos.length; i++) {
      ...
      if (art.sucursales && art.sucursales.length) {
        var sucData = art.sucursales.map(function(s) {
          return { pedido_articulo_id: artDB.id, nro_sucursal: s.nro_sucursal, cantidad: s.cantidad, estado: 'pendiente', es_entrega_final: s.nro_sucursal === '0' || s.nro_sucursal === 0 || s.es_entrega_final === true, talles: s.talles || null }
        })
